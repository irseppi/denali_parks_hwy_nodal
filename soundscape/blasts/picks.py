...
go = input("This is the script to pick new data, did you mean to run this? (y/n)")
if go.lower() != 'y':
    exit()
...
bad_data = [1149, 1266,1265,1267,1270,1261,1262,1269,1266,1258,1259,1263,1260,1264,1165,1163,1162,1211,1180,1145,1146,1291,1172,1171,1140,1151,1231,1232,1233,1227,1220,1221,1222,1217,1219,1223,1229,1224,1225,1235,1230,1228,1226,1207,1206,1209,1234,1200,1153,1152,1286]

seismo_utm_x, seismo_utm_y = zip(*[utm_proj(lon, lat) for lat, lon in zip(seismo_latitudes, seismo_longitudes)])
dist_min = np.inf
blast_num = 1
r1 = open(f'blast_{blast_num}_picks_north.txt', 'w')
if blast_num == 1:
...
for i, station in enumerate(stations):
    ...
    if Path(file).exists():
        ...
        if blast_num == 1:
            ...
            tr[2].trim(tr[2].stats.starttime + (42 * 60) + 19 ,tr[2].stats.starttime + (42 * 60) + 19 + 300)
        tr[2].filter('bandpass', freqmin=0.5, freqmax=3, corners=2, zerophase=True)
        tr[2].detrend(type='constant')
        tr[2].taper(max_percentage=0.01)
        data = tr[2][:]
        t_wf = tr[2].times()
    else:
        continue
    ...
    if seismo_latitudes[i] >= blast_lat:
        #continue
        ax.plot(t_wf, norm_data + dist_km, 'k', linewidth=0.5)
...
ax.set_ylim(19.112987073502858, 68)
plt.tight_layout()
ax.invert_yaxis()
start_time = tr[2].stats.starttime
coords = []
