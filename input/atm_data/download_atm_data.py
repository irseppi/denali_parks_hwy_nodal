...
#Created using NCPAG2S Command-Line Client: 
#https://github.com/chetzer-ncpa/ncpag2s-clc

#Root directory of the repository must be in your PATH

output_folder_path = 'atm_data/'
...
sta_f = open(f'{repo_path}input/node_crossings_db_UTM.txt','r')
...
for line in sta_f.readlines():
	...
	output = f' {output_folder_path + "/aircraft_loc/" 
			  + str(time)}_{lat}_{lon}.dat'

	# Command to download atmospheric data at the aircraft position
	comand = f'ncpag2s.py point --date {d[0:4]}-{d[4:6]}-{d[6:8]} --hour {h} '
	comand += f'--lat {lat} --lon {lon} --output {output}'
	...
	output = f'{output_folder_path}/nodes_loc/{time}_{lat_sta}_{lon_sta}.dat'

	# Command to download atmospheric data at the station position
	comand = f'ncpag2s.py point --date {d[0:4]}-{d[4:6]}-{d[6:8]} --hour {h} '
	comand += f'--lat {lat_sta} --lon {lon_sta} --output {output}'
